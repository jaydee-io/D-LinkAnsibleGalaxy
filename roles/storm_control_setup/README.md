...
|---|---|---|
| `storm_control_setup_ports` | `[]` | List of `{interface, traffic_type, level_mode, rise, low, action}` dicts |
| `storm_control_setup_polling_interval` | (none) | Polling interval in seconds |
| `storm_control_setup_polling_retries` | (none) | Number of retries |
| `storm_control_setup_loopback_enabled` | `true` | Enable loopback detection globally |
...
- hosts: dlink_switches
  roles:
    - role: jaydee_io.dlink_dgs1250.storm_control_setup
      ...
      storm_control_setup_loopback_interval: 10
      storm_control_setup_loopback_ports:
        - eth1/0/1
        - eth1/0/2
        - eth1/0/3
```
