...
def _build_commands(interface, string_value, state):
    ...
    if state == "absent":
        commands.append("no ip dhcp relay information option format-type circuit-id vendor3 string")
    else:
        commands.append("ip dhcp relay information option format-type circuit-id vendor3 string %s" % string_value)
