...
DOCUMENTATION = r"""
---
module: ddm_transceiver_monitoring_voltage
short_description: Configure transceiver monitoring voltage thresholds on a D-Link DGS-1250 switch
description:
  - Configures the C(transceiver-monitoring voltage) CLI command on a D-Link DGS-1250 switch.
  - Sets or removes the voltage threshold for a specified SFP/SFP+ port.
  - Corresponds to CLI command described in chapter 21-9 of the DGS-1250 CLI Reference Guide.
...
options:
  ...
  value:
    description:
      - The voltage threshold value in Volts (0 to 6.55).
...
EXAMPLES = r"""
- name: Set voltage low alarm threshold
  jaydee_io.dlink_dgs1250.ddm_transceiver_monitoring_voltage:
    interface: eth1/0/25
    threshold_level: low
    threshold_type: alarm
    value: 0.005

- name: Remove voltage low alarm threshold
  jaydee_io.dlink_dgs1250.ddm_transceiver_monitoring_voltage:
    interface: eth1/0/25
    threshold_level: low
    threshold_type: alarm
...
def _build_commands(interface, threshold_level, threshold_type, value, state):
    ...
    if state == "absent":
        return ["no transceiver-monitoring voltage %s %s %s" % (
            interface, threshold_level, threshold_type)]
    else:
        return ["transceiver-monitoring voltage %s %s %s %s" % (
