...
DOCUMENTATION = r"""
---
module: dot1x_initialize
short_description: Initialize 802.1X authenticator state machine on a D-Link DGS-1250 switch
description:
  - Executes the C(dot1x initialize) CLI command on a D-Link DGS-1250 switch.
  - Initializes the authenticator state machine on a specific port or for a specific MAC address.
  - Corresponds to CLI command described in chapter 3-6 of the DGS-1250 CLI Reference Guide.
version_added: "0.2.0"
author:
...
options:
  interfaces:
    description:
      - List of physical port interfaces to initialize.
  ...
  mac_address:
    description:
      - MAC address to initialize (for multi-auth mode).
...
def _build_commands(interfaces, mac_address):
    ...
    if mac_address:
        return ["dot1x initialize mac-address %s" % mac_address]

    commands = []
    for iface in interfaces:
        commands.append("dot1x initialize interface %s" % iface)
    return commands
