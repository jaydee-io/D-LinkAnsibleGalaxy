...
DOCUMENTATION = r"""
---
module: vlan_ingress_checking
short_description: Enable or disable ingress checking on a D-Link DGS-1250 switch
description:
  - Configures the C(ingress-checking) CLI command on a D-Link DGS-1250 switch.
  - Enables or disables ingress checking for frames received by a port.
  - Corresponds to CLI command described in chapter 70-2 of the DGS-1250 CLI Reference Guide.
...
EXAMPLES = r"""
- name: Enable ingress checking on port 1
  jaydee_io.dlink_dgs1250.vlan_ingress_checking:
    interface: eth1/0/1

- name: Disable ingress checking on port 1
  jaydee_io.dlink_dgs1250.vlan_ingress_checking:
    interface: eth1/0/1
    state: disabled
"""
...
def _build_commands(interface, state):
    cmd = "ingress-checking" if state == "enabled" else "no ingress-checking"
