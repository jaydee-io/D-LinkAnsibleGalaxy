...
DOCUMENTATION = r"""
---
module: igmp_snooping_static_group
short_description: Configure IGMP snooping static group on a D-Link DGS-1250 switch
description:
  - Configures the C(ip igmp snooping static-group) CLI command on a D-Link DGS-1250 switch.
  - Adds or removes an IGMP snooping static group on a VLAN.
...
options:
  ...
  group_address:
    description:
      - The IP multicast group address (e.g. C(226.1.2.3)).
    type: str
    required: true
  interface_id:
    description:
...
EXAMPLES = r"""
- name: Add IGMP snooping static group on VLAN 1
  jaydee_io.dlink_dgs1250.igmp_snooping_static_group:
    vlan_id: 1
    group_address: "226.1.2.3"
    interface_id: eth1/0/5

- name: Remove IGMP snooping static group from VLAN 1
  jaydee_io.dlink_dgs1250.igmp_snooping_static_group:
    vlan_id: 1
    group_address: "226.1.2.3"
    state: absent
