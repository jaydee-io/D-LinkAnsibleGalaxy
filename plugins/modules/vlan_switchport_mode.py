...
DOCUMENTATION = r"""
---
module: vlan_switchport_mode
short_description: Configure VLAN port mode on a D-Link DGS-1250 switch
description:
  - Configures the C(switchport mode) CLI command on a D-Link DGS-1250 switch.
  - Sets the VLAN mode (access, hybrid, trunk) for an interface.
  - Corresponds to CLI command described in chapter 70-7 of the DGS-1250 CLI Reference Guide.
...
options:
  ...
  state:
    description:
      - C(present) to set, C(absent) to revert to default (hybrid).
    type: str
...
def main():
    module = AnsibleModule(
        argument_spec=dict(
            ...
            mode=dict(type="str", choices=["access", "hybrid", "trunk"]),
