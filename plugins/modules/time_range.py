...
---
module: time_range
short_description: Create or delete a time range profile on a D-Link DGS-1250 switch
description:
  - Configures the C(time-range) CLI command on a D-Link DGS-1250 switch.
  - Creates or deletes a time range profile.
  - Corresponds to CLI command described in chapter 68-3 of the DGS-1250 CLI Reference Guide.
...
EXAMPLES = r"""
- name: Create time range profile
  jaydee_io.dlink_dgs1250.time_range:
    name: rdtime

- name: Delete time range profile
  jaydee_io.dlink_dgs1250.time_range:
    name: rdtime
    state: absent
"""
...
def _build_commands(name, state):
    if state == "absent":
        return ["no time-range %s" % name]
    return ["time-range %s" % name, "exit"]
