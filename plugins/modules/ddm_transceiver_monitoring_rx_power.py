...
DOCUMENTATION = r"""
---
module: ddm_transceiver_monitoring_rx_power
short_description: Configure transceiver monitoring RX power thresholds on a D-Link DGS-1250 switch
description:
  - Configures the C(transceiver-monitoring rx-power) CLI command on a D-Link DGS-1250 switch.
  - Sets or removes the input power threshold for a specified SFP/SFP+ port.
  - Corresponds to CLI command described in chapter 21-6 of the DGS-1250 CLI Reference Guide.
...
EXAMPLES = r"""
- name: Set RX power low warning threshold in mwatt
  jaydee_io.dlink_dgs1250.ddm_transceiver_monitoring_rx_power:
    interface: eth1/0/25
    threshold_level: low
...
- name: Remove RX power low warning threshold
  jaydee_io.dlink_dgs1250.ddm_transceiver_monitoring_rx_power:
    interface: eth1/0/25
    threshold_level: low
    threshold_type: warning
...
def _build_commands(interface, threshold_level, threshold_type, unit, value, state):
    ...
    if state == "absent":
        return ["no transceiver-monitoring rx-power %s %s %s" % (
            interface, threshold_level, threshold_type)]
    else:
        return ["transceiver-monitoring rx-power %s %s %s %s %s" % (
