...
---
module: dgs1250_dns
short_description: Manage DNS name servers on a D-Link DGS-1250 switch
description:
  - Manage DNS name server configuration on D-Link DGS-1250 switches.
...
options:
  config:
    description:
      - A list of DNS name server configurations.
    type: list
    elements: dict
    suboptions:
      address:
  ...
  state:
    description:
      - The desired state of the DNS server configuration.
    type: str
...
- name: Configure DNS servers
  jaydee_io.dlink_dgs1250.dgs1250_dns:
    config:
      - address: 8.8.8.8
      - address: 8.8.4.4
    state: merged

- name: Override DNS servers
  jaydee_io.dlink_dgs1250.dgs1250_dns:
    config:
      - address: 1.1.1.1
    state: overridden

- name: Gather DNS server config
  jaydee_io.dlink_dgs1250.dgs1250_dns:
    state: gathered
"""
...
RETURN = r"""
before:
  description: DNS server configuration before the changes.
  returned: always (except gathered)
  type: list
  elements: dict
after:
  description: DNS server configuration after the changes.
  returned: when changed
  type: list
  elements: dict
gathered:
  description: Current DNS server configuration gathered from the switch.
...
def _parse_dns_servers(config):
    servers = []
    for line in config.splitlines():
        m = re.match(r"^\s*ip name-server\s+(\S+)\s*$", line)
        if m:
...
def _build_commands_merged(want, have_addrs):
    ...
    for entry in want:
        if entry["address"] not in have_addrs:
            commands.append("ip name-server %s" % entry["address"])
    return commands
...
def _build_commands_overridden(want, have_addrs):
    ...
    for addr in sorted(have_addrs):
        if addr not in want_addrs:
            commands.append("no ip name-server %s" % addr)
    for entry in want:
        if entry["address"] not in have_addrs:
            commands.append("ip name-server %s" % entry["address"])
    return commands
    ...
    if not want:
        ...
        for entry in want:
            if entry["address"] in have_addrs:
                commands.append("no ip name-server %s" % entry["address"])
    return commands
